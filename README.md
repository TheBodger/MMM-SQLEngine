...
This module requires both MMM-Utilities and MMM-Structures to be installed.

Before installing this module;
		Install https://github.com/TheBodger/MMM-Utilities 
		Install https://github.com/TheBodger/MMM-Structures 
...
File feeds are better for debugging other modules, ensuring that the input data is always fixed. File feed are read in when the module first runs and is **not refreshed**. 
An input feed from another module running in the current instance of MM will be processed as it is published to this module
 
Once the data has been read, it will be stored in a temporary memory based SQLite DB ready for use in the SQL query, see below in output feeds for more details.
...
The data is formatted either as NDTF or RSS.
...
Note: options prefixed with # are not yet available.
...
| `showDom`         | *Optional* - If true, the output from the formatted query results is displayed on the MM display. if dhowdom is true, the module must have a position varible set.<br><br> **Possible values:** true or false<br> **Default value:** false
...
this configuration produces a single NDTF feeds from an input feed from a Provider-json2 module. See the Petroo price option in provider-json2 for a query using an external database. 
...
Also check the MMM-Provider-JSON2 module for an example config that extracts UK Petrol station prices, addresses and locations that are then processed in the SQL engine to produce a RSS feed for display. There is also an example to process share prices and currency conversion rates to give locally prices current share prices.

### Additional Notes

This is a WIP; changes are being made all the time to improve the compatibility across the modules. Please refresh this and the MMM-Utilities and MMM-Structures modules with a `git pull` in the relevant modules folders.
...
A node-js application is included that will create and load data from the postoffic postcode database into a filebased sqlite DB.
